    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-400 mt-1">Monitor DNS query traffic and performance</p>
        </div>
        <select
          value={selectedZone}
      ...
      <div className="grid gap-6 lg:grid-cols-2 mb-6">
        <div className="bg-gray-800 rounded-lg p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Query Types</h2>
          <div className="space-y-3">
            ...
            {!queryTypes && (
              <p className="text-gray-400 text-center py-4">No query data available</p>
            )}
          </div>
        </div>
            ...
            {showGlobalStats && stats?.top_queried_domains?.map((item: { domain: string; count: number }, index: number) => (
              <div key={item.domain} className="flex items-center justify-between py-2">
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full bg-teal-500/20 text-teal-400 flex items-center justify-center text-sm">
                    {index + 1}
                  </span>
                  <span className="text-white font-mono text-sm">{item.domain}</span>
                </div>
                <span className="text-gray-400">{item.count.toLocaleString()} queries</span>
              </div>
            ))}
      ...
      {showZoneStats && zoneStats?.geo_distribution && Object.keys(zoneStats.geo_distribution).length > 0 && (
        <div className="bg-gray-800 rounded-lg p-6">
          <h2 className="text-lg font-semibold text-white mb-4">Query Sources by Region</h2>
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
            {Object.entries(zoneStats.geo_distribution).map(([region, count]) => (
              <div key={region} className="bg-gray-900 rounded-lg p-4 text-center">
                ...
                <p className="text-2xl font-bold text-white mt-1">{(count as number).toLocaleString()}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {showGlobalStats && stats && (
        <div className="bg-gray-800 rounded-lg p-6 mt-6">
          <h2 className="text-lg font-semibold text-white mb-4">Security Metrics</h2>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="bg-gray-900 rounded-lg p-4">
              <p className="text-gray-400 text-sm">Rate Limited Queries</p>
              <p className="text-2xl font-bold text-red-400 mt-1">
                {stats.rate_limited_queries.toLocaleString()}
              </p>
            </div>
            <div className="bg-gray-900 rounded-lg p-4">
              <p className="text-gray-400 text-sm">DNSSEC Verified Responses</p>
